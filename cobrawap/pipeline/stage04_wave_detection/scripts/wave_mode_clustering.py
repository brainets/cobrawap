...
def build_timelag_dataframe(waves_evt, normalize=True):
    ...
    timelag_df = pd.DataFrame(
        timelag_matrix, index=wave_ids, columns=channel_ids
    )
    ...
    for i, trigger in enumerate(waves_evt):
        ...
        if np.isnan(timelag_df[channel_id][wave_id]):
            timelag_df[wave_id, channel_id] = trigger.magnitude
...
def fill_nan_sites_from_similar_waves(
    ...
    for row, wave_id in enumerate(timelag_df.index):
        ...
        neighbourhood_distance[row] = np.mean(
            neighbour_distances[:num_neighbours]
        )
        neighbour_pos = np.array(
            [
                pair_indices[0][i]
                if pair_indices[0][i] != row
                else pair_indices[1][i]
                for i in pair_pos[sort_idx]
            ]
        )
...
def get_triu_indices_pos(i, N):
    ...
    idx0 = np.array(
        [k for k in range(idx0_start(i), idx0_start(i + 1))], dtype=int
    )
    idx1 = np.array(
        [i - 1 + np.sum([(N - 2 - j) for j in range(k)]) for k in range(i)],
        dtype=int,
    )
...
def pca_transform(timelag_matrix, dims=None):
    if dims is not None and dims > len(timelag_df):
        warn(
            f"Too few waves ({len(timelag_df)}) to perform a pca reduction "
...
def kmeans_cluster_waves(timelag_matrix, n_cluster=7):
    ...
    if n_cluster > n_waves:
        warn(
            f"Too few waves {n_waves} to determine {n_cluster} "
...
def build_cluster_timelag_dataframe(timelag_df, cluster_labels):
    cluster_timelag_df = pd.DataFrame(
        index=np.unique(cluster_labels), columns=timelag_df.columns
    )
...
def wave_to_grid(wave_evt):
    ...
    annotation_idx = [
        np.argmax(wave_evt.array_annotations["channels"] == channel)
        for channel in channels
    ]
    x_coords = wave_evt.array_annotations["x_coords"][annotation_idx].astype(
        int
    )
    y_coords = wave_evt.array_annotations["y_coords"][annotation_idx].astype(
        int
    )
...
def plot_wave_modes(wavefronts_evt, wavemodes_evt):
    ...
    for i, mode_id in enumerate(wavemodes_evt.annotations["mode_labels"]):
        ...
        waves = wavefronts_evt[
            wavefronts_evt.array_annotations["wavemode"] == mode_id
        ]
        ...
        if dim_x == 1:
            mode_grid = np.stack(
                (np.squeeze(mode_grid), np.squeeze(mode_grid)), axis=1
            )
        ...
        ax.set_yticklabels([])
        ax.set_xlabel(
            f"mode #{mode_id} | {mode_count} waves | var={mode_dist:.2f}"
        )

    cbar = plt.colorbar(
        img, cax=axes[-1], ticks=[-vminmax / 1.5, vminmax / 1.5]
    )
    cbar.ax.set_yticklabels(
        ["wave start", "wave end"], rotation=90, va="center"
    )
    return None


def clean_timelag_dataframe(
    df,
    ...
    df = fill_nan_sites_from_similar_waves(
        df,
...
if __name__ == "__main__":
    ...
    if len(mode_ids) != len(timelag_df):
        raise IndexError(
            "Some waves are not assigned to a kmeans cluster!"
    ...
    for i, cluster_grid in enumerate(mode_grids):
        ...
        fx, fy, pattern = sample_wave_pattern(
            pattern_func,
        ...
        interpolated_mode_grids = (
            np.concatenate((interpolated_mode_grids, pattern[np.newaxis, :]))
            if i
            else pattern[np.newaxis, :]
        )
    ...
    plot_wave_modes(
        wavefronts_evt=block.segments[0].events[evt_id], wavemodes_evt=evt
    )
    if args.output_img is not None:
        save_plot(args.output_img)
