...
def horn_schunck_step(
    ...
    for i in range(max_Niter):
        ...
        # check convergence
        max_dv = np.max(
            [np.max(np.abs(vx - new_vx)), np.max(np.abs(vy - new_vy))]
        )
        vx, vy = new_vx, new_vy
        if max_dv < convergence_limit:
            break

    vector_frame = vx + vy * 1j

    if nan_channels:
        vector_frame[nan_channels] = np.nan + np.nan * 1j

    return vector_frame


def compute_derivatives(
    frame, next_frame, kernelX, kernelY, kernelT, are_phases=False
):
    if are_phases:
        fx = phase_conv2d(frame, kernelX) + phase_conv2d(next_frame, kernelX)
        fy = phase_conv2d(frame, kernelY) + phase_conv2d(next_frame, kernelY)
...
def horn_schunck(
    ...
    _vector_frames = Parallel(n_jobs=n_jobs)(
    ...
    for n_i, i in enumerate(_vector_frames):
        vector_frames[n_i] = i
    del _vector_frames
...
def smooth_frames(frames, sigma):
    ...
    if np.isfinite(frames).any():
        ...
        if np.iscomplexobj(frames):
            frames[:, nan_sites[0], nan_sites[1]] = (
                np.nanmedian(np.real(frames))
    ...
    if np.iscomplexobj(frames):
        frames = (
            gaussian_filter(np.real(frames), sigma=sigma, mode="nearest")
            + gaussian_filter(np.imag(frames), sigma=sigma, mode="nearest")
            * 1j
        )
    else:
...
def is_phase_signal(signal, use_phases):
    ...
    in_range = np.isclose(
        np.array([vmin, vmax]), np.array([-np.pi, np.pi]), atol=0.05
    )
    if in_range.all():
        ...
        if use_phases:
            ...
            print(
                "Anyhow, the signal is treated as phase signal in the following. If this is not desired please review the preprocessing."
            )
        return True
    else:
        return False
